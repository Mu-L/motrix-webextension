...
test.describe('Filename Extraction', () => {
  test('distinctive Content-Disposition filename is forwarded to aria2 options', async () => {
    ...
    // If Chrome resolved the name from Content-Disposition, it should match.
    // Some Chrome versions use a temp UUID — in that case, just having a non-empty
    // out is the important thing (covered by the assertions above).
    if (isExpectedName) {
      expect(filename).toBe('test-report-2024.pdf');
    }
