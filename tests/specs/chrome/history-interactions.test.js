...
test.describe('Multiple Downloads Listed', () => {
  test('three downloads all appear in history', async () => {
    // Seed 3 download entries and open history page
    const histPage = await context.newPage();

    // Navigate to config to seed storage, then navigate to history
    await histPage.goto(`chrome-extension://${extensionId}/pages/config.html`, { waitUntil: 'domcontentloaded' });
    ...
    await histPage.goto(`chrome-extension://${extensionId}/pages/history.html`, { waitUntil: 'networkidle' });

    // Wait for React to render all 3 entries
    await waitFor(async () => {
