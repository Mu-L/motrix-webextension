...
test.describe('Download List Rendering', () => {
  test('intercepted download appears in popup with filename', async () => {
    ...
    const downloadTexts = page.locator('.text');
    await expect(downloadTexts.first()).toBeVisible({ timeout: 10_000 });
    const count = await downloadTexts.count();
    expect(count).toBeGreaterThan(0);

    // The filename should be present somewhere
