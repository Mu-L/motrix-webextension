...
// ══════════════════════════════════════════════════════════════════════════════
// 3. Aria2 Connection Failure Mid-Download
// ══════════════════════════════════════════════════════════════════════════════
test.describe('Aria2 Connection Failure Mid-Download', () => {
  test('download status becomes error after consecutive poll failures', async () => {
    const page = await openFileServerPage();
    try {
      await page.click('#large-download');
      await waitFor(() => mockAria2.getCalls('addUri').length > 0, 25_000);

      // Wait for tracking to start (first tellStatus), then sever connection
      await waitFor(() => mockAria2.getCalls('tellStatus').length > 0, 10_000, 200, 'first tellStatus');
      mockAria2.setRejectConnections(true);

      // AriaTracker polls every 1s, needs 3 consecutive failures → ~4s minimum
      await waitFor(async () => {
        const downloads = await readLocalStorage('downloads');
        if (!downloads) return false;
        const entries = Object.values(downloads);
        return entries.some((entry) => entry.status === 'error');
      }, 20_000, 500, 'download status to become error');
